...
AtomicStack* CreateAtomicStack ()
{
	...
	return (AtomicStack*) UNITY_NEW_ALIGNED (AtomicStack, kMemThread, sizeof(atomic_word2));
#elif UNITY_PS3 || UNITY_XENON
	return (AtomicStack*) UNITY_NEW_ALIGNED (AtomicStack, kMemThread, 64);	//	Reservation size safe
#else
	return (AtomicStack*) UNITY_NEW_ALIGNED (AtomicStack, kMemThread, sizeof(atomic_word));
#endif
}
...
#if defined (ATOMIC_HAS_DCAS)
#if UNITY_XENON
	AtomicNode* dummy = UNITY_NEW_ALIGNED (AtomicNode, kMemThread, 64);
#else
	AtomicNode* dummy = UNITY_NEW (AtomicNode, kMemThread);
#endif
...
#if UNITY_PS3
	AtomicNode* dummy = UNITY_NEW_ALIGNED(AtomicNode, kMemThread, 64);	//	Reservation size safe
#else
	AtomicNode* dummy = UNITY_NEW (AtomicNode, kMemThread);
#endif
...
AtomicQueue* CreateAtomicQueue ()
{
	// should be properly aligned
#if defined (ATOMIC_HAS_DCAS)
	return (AtomicQueue*) UNITY_NEW_ALIGNED (AtomicQueue, kMemThread, sizeof(atomic_word2));
#else
	return (AtomicQueue*) UNITY_NEW_ALIGNED (AtomicQueue, kMemThread, sizeof(atomic_word));
#endif
}
...
{
#if UNITY_PS4
	//	The jobs system is carefuly balanced against cores with mono threads. We can end up with significant stalls if we don't let other equal priority threads run. A yield here gives best performance
	PS4AtomicListRelax();
